...
#line 0 "../addons-all/ofxNanoVG/libs/nanovg/src/nanovg.h"
//
...
#line 0 "../addons-all/ofxNanoVG/src/ofxNanoVG.h"
//
//  ofxNanoVG.h
//  sentopiary
//
...
public:
	 ...
	 * Style
	 */
	inline void setStrokeWidth(float width) { nvgStrokeWidth(ctx, width); }
	inline void setLineCap(enum LineParam cap) { nvgLineCap(ctx, cap); }
	inline void setLineJoin(enum LineParam join) { nvgLineJoin(ctx, join); }
	inline void setFillColor(const ofFloatColor &c) { nvgFillColor(ctx, toNVGcolor(c)); }
	inline void setFillPaint(const NVGpaint &paint) { nvgFillPaint(ctx, paint); }
	inline void setStrokeColor(const ofFloatColor &c) { nvgStrokeColor(ctx, toNVGcolor(c)); }
	inline void setStrokePaint(const NVGpaint &paint) { nvgStrokePaint(ctx, paint); }
	inline NVGpaint getLinearGradientPaint(float sx, float sy, float ex, float ey, const ofColor &c1, const ofColor &c2) { return nvgLinearGradient(ctx, sx, sy, ex, ey, toNVGcolor(c1), toNVGcolor(c2)); }

	NVGpaint getTexturePaint(const ofTexture& tex);
	static inline NVGcolor toNVGcolor(const ofFloatColor& c) { return nvgRGBAf(c.r, c.g, c.b, c.a); }
