...
parserutils_error parserutils_buffer_create(parserutils_buffer **buffer)
{
	...
	b = (parserutils_buffer *)malloc(sizeof(parserutils_buffer));
	if (b == NULL)
		return PARSERUTILS_NOMEM;

	b->data = (uint8_t *)malloc(DEFAULT_SIZE);
	if (b->data == NULL) {
		free(b);
		return PARSERUTILS_NOMEM;
	}
...
{
	uint8_t *temp = (uint8_t *)realloc(buffer->data, buffer->allocated * 2);
	if (temp == NULL)
		return PARSERUTILS_NOMEM;
...
#ifndef NDEBUG
	temp = (uint8_t *)malloc(buffer->allocated);
	if (temp == NULL)
		return PARSERUTILS_NOMEM;
		...
		parserutils_stack **stack)
{
	...
	s = (parserutils_stack *)malloc(sizeof(parserutils_stack));
	if (s == NULL)
		return PARSERUTILS_NOMEM;
		...
		size_t chunk_size, parserutils_vector **vector)
{
	...
	v = (parserutils_vector *)malloc(sizeof(parserutils_vector));
	if (v == NULL)
		return PARSERUTILS_NOMEM;
