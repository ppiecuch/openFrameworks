		...
		parserutils_filter **filter)
{
	...
	f = (parserutils_filter *)malloc(sizeof(parserutils_filter));
	if (f == NULL)
...
typedef struct parserutils_inputstream_private {
	parserutils_inputstream pub;	/**< Public part. Must be first */
		...
		parserutils_inputstream **stream)
{
	...
	s = (parserutils_inputstream_private *)malloc(sizeof(parserutils_inputstream_private));
	if (s == NULL)
		return PARSERUTILS_NOMEM;
	...
	error = parserutils_buffer_create(&s->pub.utf8);
	if (error != PARSERUTILS_OK) {
	...
	if (error != PARSERUTILS_OK) {
		parserutils_buffer_destroy(s->pub.utf8);
		parserutils_buffer_destroy(s->raw);
		free(s);
		return error;
	}
		...
		const uint8_t *data, size_t len)
{
	...
	return parserutils_buffer_insert(s->pub.utf8, s->pub.cursor,
			data, len);
}
		...
		size_t offset, const uint8_t **ptr, size_t *length)
{
	...
	if (s->raw->length == 0) {
		...
		return s->pub.had_eof ? PARSERUTILS_EOF
	...
	if (s->pub.cursor + offset == s->pub.utf8->length)
		return PARSERUTILS_NEEDDATA;

	/* Now try the read */
	if (IS_ASCII(s->pub.utf8->data[s->pub.cursor + offset])) {
		len = 1;
	} else {
		error = parserutils_charset_utf8_char_byte_length(
			s->pub.utf8->data + s->pub.cursor + offset,
			&len);
	...
	(*length) = len;
	(*ptr) = (s->pub.utf8->data + s->pub.cursor + offset);
		...
		parserutils_inputstream_private *stream)
{
	...
	if (stream->pub.cursor == stream->pub.utf8->length) {
		/* Cursor's at the end, so simply reuse the entire buffer */
		utf8 = stream->pub.utf8->data;
		utf8_space = stream->pub.utf8->allocated;
	} else {
		/* Cursor's not at the end, so shift data after cursor to the
		 * bottom of the buffer. If the buffer's still over half full,
		 * extend it. */
		memmove(stream->pub.utf8->data,
			stream->pub.utf8->data + stream->pub.cursor,
			stream->pub.utf8->length - stream->pub.cursor);

		stream->pub.utf8->length -= stream->pub.cursor;

		if (stream->pub.utf8->length >
				stream->pub.utf8->allocated / 2) {
			error = parserutils_buffer_grow(stream->pub.utf8);
			if (error != PARSERUTILS_OK)
				return error;
		}

		utf8 = stream->pub.utf8->data + stream->pub.utf8->length;
		utf8_space = stream->pub.utf8->allocated -
				stream->pub.utf8->length;
	}
	...
	stream->pub.utf8->length =
			stream->pub.utf8->allocated - utf8_space;
