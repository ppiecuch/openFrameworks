...
expat_xmlparser_start_element_handler(void *_parser,
				      ...
				      const XML_Char **atts)
{
	dom_xml_parser *parser = (dom_xml_parser *)_parser;
	dom_exception err;
	...
	if (ns == NULL)
		...
		err = dom_document_create_element_ns(parser->doc, ns,
						     tag_name, &elem);
	if (err != DOM_NO_ERR) {
		if (ns != NULL)
			dom_string_unref(ns);
	...
	while (*atts) {
		...
		if (ns == NULL)
			...
			err = dom_element_set_attribute_ns(elem, ns,
							   key, value);
	        if (ns != NULL)
			dom_string_unref(ns);
		dom_string_unref(key);
			      ...
			      int len)
{
	...
	dom_node_type ntype = (dom_node_type)0;
		      ...
		      dom_msg msg, void *mctx, dom_document **document)
{
	...
	parser = (dom_xml_parser *)calloc(sizeof(*parser), 1);
	if (parser == NULL) {
...
dom_xml_parser_parse_chunk(dom_xml_parser *parser, uint8_t *data, size_t len)
{
	...
	if (status != XML_STATUS_OK) {
		...
		return (dom_xml_error)(DOM_XML_EXTERNAL_ERR | status);
	}
