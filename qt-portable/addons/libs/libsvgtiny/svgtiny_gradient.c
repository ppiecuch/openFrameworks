		...
		struct svgtiny_parse_state *state)
{
	...
	for (j = 0; j != n; ) {
		...
		point = (struct grad_point *)svgtiny_list_push(pts);
		if (!point) {
			free(p);
	...
	while (a != b) {
		struct grad_point *point_t = (struct grad_point *)svgtiny_list_get(pts, t);
		struct grad_point *point_a = (struct grad_point *)svgtiny_list_get(pts, a);
		struct grad_point *point_b = (struct grad_point *)svgtiny_list_get(pts, b);
